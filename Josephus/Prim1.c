...
void prim(Graph G, int start)
{
	...
	//顶点个数次的循环,每次选择一个添加一个相对选定顶点最小权值的顶点添加到prims数组中,直到所有结点都被添加到prims数组中
	for (i = 0; i < G.vexnum; i++)
	{
		...
		j = 0;//顶点个数循环变量
		k = 0;//在当前第i个顶点下除去已经选择的顶点外到当前顶点权值最小的结点的数组索引
		min = INF;
		...
		//最新加入prims数组的结点是第k个结点,用第k个结点到其他所有未加入prims数组的结点的权值进行比较,选择较小的权值更新weights数组对应的结点
		for (j = 0; j < G.vexnum; j++)
		{
